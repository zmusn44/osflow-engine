...
public class R_S003_B019 implements LinkeyRule {
    ...
    public String run(HashMap<String, Object> params) throws Exception {
        ...
        if (BeanCtx.getLinkeywf().isDebug()) {
            logger.info("运行userTask节点=" + runNodeid + ",是否使用Post用户作为参与者=" + usePostOwnerFlag + ",自动运行标识=" + userTaskAutoRunFlag);
        }
        if (usePostOwnerFlag.equals("1")) {
            ...
            if (userTaskAutoRunFlag.equals("1")) {
                ...
                startUserList = Tools.join(Tools.splitAsLinkedSet(startUserList), ",");//分析用户字符串
        ...
        if (BeanCtx.getLinkeywf().isDebug()) {
            logger.info("启动节点" + runNodeid + "用户startUserList=" + startUserList);
            logger.info("启动节点" + runNodeid + "指定用户所属部门userDeptSet=" + userDeptSet);
        }
