...
public class ProcessEngine {
    ...
    private Rdb rdb = BeanCtx.getRdb(); //从上下文中获取Rdb接口实例
    ...
    public void init(String processid, String docUnid, String userid, String taskid) throws Exception {
        ...
        if (Tools.isBlank(docUnid)) {
            ...
            this.docUnid = docUnid; //使用传入进来的文档id         // Mr.Yun 暂时不需要
        ...
        //-Mr.Yun- formDoc = ((ModForm) BeanCtx.getBean("ModForm")).getFormDoc(formNumber); //formNumber在initProcessVar()方法中给值
        ...
        // 这里需要考虑如何将主表的数据更新到主文档中的
        if(BeanCtx.getMainData() == null){
            BeanCtx.setMainData(new JSONObject());
        }
        this.document.appendFromJsonStr(BeanCtx.getMainData().toJSONString()); //把移动前端请求的数据也初始化到主文档中去
        //this.document.appendFromRequest(BeanCtx.getRequest(), true); //Mr.YUn 把请求的数据也初始化到请求中去
    ...
    public String run(String actionid, HashMap<String, Object> runParams) throws Exception {
        ...
        if (Tools.isNotBlank(this.getEndNodeid())) {
            ...
            if (this.processModNodedoc.g("NullUserErrorFlag").equals("Y") && Tools.isBlank(this.document.g("WF_Author"))) {
                ...
                //发送待办待阅队例
                /** 暂不支持全局配置项，一般流程正常流转也不会执行到这里面的方法
        ...
        // 20190723 添加额外增加的 流程引擎增加同步任务 --Start
        String syncRuleNum = ruleConfig.getRuleNumByEngineActionid("SyncUserTask");
        if (Tools.isNotBlank(syncRuleNum)) {
            // 如果这个规则编号不为空，执行配置的规则
            BeanCtx.getExecuteEngine().run(syncRuleNum, runParams);
        }
        // 20190723 添加额外增加的 流程引擎增加同步任务 --End
...
//    }
    ...
    public String getRunMsg() {
        ...
        if (this.getEndNodeid() == null) {
            ...
            if (Tools.isNotBlank(nextUserList)) {
                // nextUserList = BeanCtx.getLinkeyUser().getCnName(nextUserList); // Mr.Yun不支持组织内容
            }
    ...
    public void saveStayData(String processid, String nodeid, String docUnid, HashMap<String, Object> params) {
        ...
        for (String nextNodeid : nextNodeList) {
            ...
            // 20200922 BeanCtx.getLinkeywf().getDocument().g("WF_SendSms") , 这里暂不支持短信的判断
            doc.s("WF_SendSms", (String) params.get("WF_SendSms"));
