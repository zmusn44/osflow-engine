...
public class RemarkImpl implements Remark {
    ...
    public String getRemarkList(String docUnid, String remarkType, String remarkTable, String isReadFlag) {
        ...
        String remarkStyle = ""; //不支持从配置表中读取，使用默认
    ...
    public int AddReadRemark(String nodeid, String nodeName, String startTime) {
        ...
        //doc.s("Deptid", BeanCtx.getLinkeyUser().getDeptidByUserid(BeanCtx.getUserid(), false));//不支持部门
        doc.s("Deptid", "");
        //doc.s("DeptName", BeanCtx.getLinkeyUser().getFullDeptNameByDeptid(doc.g("Deptid"), false));//不支持部门
        doc.s("DeptName", "");
        doc.s("Remark", "已阅");
        doc.s("ActionName", "阅读");
        doc.s("Actionid", "Read");
    ...
    public int AddRemark(String actionid, String remark, String remarkLevel) {
        ...
        if (Tools.isNotBlank(nextUserList)) {
            //nextUserList = linkeyUser.getCnName(nextUserList); //转换成为中文名称//不支持
        }
        ...
        doc.s("Userid", BeanCtx.getUserid());
        //doc.s("UserName", linkeyUser.getCnName(BeanCtx.getUserid()));//不支持
        doc.s("UserName", BeanCtx.getUserName());
        ...
        //doc.s("Deptid", linkeyUser.getDeptidByUserid(BeanCtx.getUserid(), false));//不支持
        //doc.s("DeptName", linkeyUser.getFullDeptNameByDeptid(doc.g("Deptid"), false));//不支持
