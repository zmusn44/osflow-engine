...
public class ModNode {
    ...
    public Document getNodeDoc(String processid, String nodeid) {
        ...
        if (nodeDoc.isNull()) {
            BeanCtx.log("E", "ModNode.getNodeDoc", "ModNode.getNodeDoc() Error:Can't find the node document by sql(" + sql + ")");
        }
        return nodeDoc;
    }
     ...
     * @param sourceNodeid 节点id
     * @param isCondition true表示条件表达式不符合的不返回，false表示返回全部
     * @return 返回下一个节点id hashset集合
     * @throws Exception 获取下一个节点出错
     */
    ...
    public boolean checkSequenceFlow(Document nodeDoc) throws Exception {
        ...
        if (Tools.isNotBlank(conditionRuleNum)) {
            ...
            // 直接运算条件
            //((ScriptEngine) BeanCtx.getBean("ScriptEngine")).evalRouterCondition(conditionExpression);//-Mr.Yun-不支持路由运算，直接反回false
            return false;
        }
    }
    ...
    public HashMap<String, String> getTaskUser(String processid, HashSet<String> nodeidList) throws Exception {
        ...
        for (String nodeid : nodeidList) {
            ...
            //potentialOwner = ((LinkeyUser) BeanCtx.getBean("LinkeyUser")).parserNodeMembers(potentialOwner);//不支持组织结构
            potentialOwner = this.parserNodeMembers(potentialOwner);
     ...
     *     Mr.Yun 注：这个方法是LinkeyUser.parserNodeMembers迁移过来的，
     *     简化了组织这一部分对Role、Dept的计算，遇到这一类计算，直接反回空，即不支持，所以方法会有一些问题
    ...
    public LinkedHashSet<String> parserNodeMembers(Set<String> potentialOwner) throws Exception {
        ...
        for (String userRule : potentialOwner) {
            ...
            if (Tools.isBlank(ruleType)) {
                //这里面有可能是部门，角色，岗位，还不一定就是用户
                //userSet.addAll(this.parserUserid(userRule));
                //这里不考虑支持组织的情况，所以忽略，认为就是用户了
                userSet.add(userRule);
            }
                ...
                if (epos != -1) {
                    ...
                    if (userid.equals("CurrentUser")) {
                        //当前用户
                        // userSet.addAll(getRoleMemberByUserid(BeanCtx.getUserid(), roleNumber)); //不支持的计算
                    }
                        ...
                        userSet.add(userid);//这里仅考虑用户，而不考虑角色
                    }
                        ...
                        userSet.addAll(insNodeUserSet);//仅考虑用户的情况
                    }
                    /** 不考虑部门的情况
                ...
                //userSet.addAll(this.parserUserid(userRule)); //这里面有可能是部门，角色，岗位，还不一定就是用户，暂不考虑组织结构
                userSet.add(userRule);
            }
        }
        userSet.remove("");
