...
public class InsUser {
     ...
     * @param backFlag 回退标识，1标识回退，2标识回退后需返回给回退者
    ...
    public int endUser(String processid, String docUnid, String nodeid, String userList) throws Exception {
        ...
        for (String userid : userArray) {
            ...
            insUserDoc.s("ActionUserid", BeanCtx.getUserid());
            insUserDoc.s("Status", "End");
            // 20200922，这里对取消了BeanCtx.g()，req中的参数的函数，改成从主表单中的字段进行取参数值
            insUserDoc.s("IsAgree", BeanCtx.getLinkeywf().getDocument().g("WF_IsAgree")); //会签时是否同意选项 WF_IsAgree
...
//        String nodeType = ((ModNode) BeanCtx.getBean("ModNode")).getNodeType(processid, nodeid);
        ...
        for (String userid : userArray) {
            ...
            if (!rdb.hasRecord(sql)) {
                ...
                if (!rdb.hasRecord(sql)) {
                    ...
                    insUserDoc.s("Userid", userid);
                    insUserDoc.s("ActionUserid", BeanCtx.getUserid());
                    //insUserDoc.s("Deptid", linkeyuser.getDeptidByUserid(userid, false));//不支持组织结构
                    insUserDoc.s("Deptid", "");
    ...
    public Document getInsUserDoc(String processid, String nodeid, String docUnid, String userid, String status) {
        ...
        if (status.indexOf(",") == -1) {
            ...
            // Mr.Yun 添加批注：存在多个状态，这么写应该是有一些问题的，使用in语句会好很多，但考虑到只反回一个用户实例，也不见有多大的风险
     ...
     * @param processid 流程id
     * @param nodeid 节点id
     * @param insUserDoc 当前用户实例
     * @param modNodeDoc 模型节点
