...
public class MainDoc {
    ...
    public String saveDocument() throws Exception {
        ...
        document.s("WF_Author_CN", author); // 审批用户中文名，默认为用户工号
        ...
        if (Tools.isBlank(document.g("WF_DocNumber"))) {
            // ENGINEDOCUMENT 这里更改成 时间做为流水号 yyMMddHHmmss+三个随机整数
            document.s("WF_DocNumber", Tools.getNewSerialNo());
        }

        /* 运行主表单存盘事件 */
        /** Mr.Yun 暂不支持表单，和表单的事件
         String returnMsg = "1";
        ...
        // 由于不支持绑定表单，所以linkeywf.getFormDoc()是一个空对象，会报错，后续支持时，再打开
    ...
    public int archiveDocument() throws Exception {
        ...
        // 20181129 Mr.Yun 修改归档业务类别中Add Start
        if(Tools.isNotBlank(document.g("WF_EndBusinessName"))){
            endBusinessName = document.g("WF_EndBusinessName");
        }
        if(Tools.isNotBlank(document.g("WF_EndBusinessid"))){
            endBusinessid = document.g("WF_EndBusinessid");
        }
        // 20181129 Mr.Yun 修改归档业务类别中Add End
    ...
    protected int archiveRemarkList(String docUnid) {
        if (rdb.hasRecord("select WF_OrUnid from BPM_InsRemarkList where DocUnid='" + docUnid + "'")) {
            ...
            if (i < 1) {
                BeanCtx.log("E", "MainDoc.archiveRemarkList","归档流转记录时出错,记录没有插入到BPM_ArchivedRemarkList中(" + sql + ")");
            }
            return i;
        }
        else {
    ...
    private int archiveModGraphic(String processid, String docUnid) {
        // 这里修改为false状态，不再从配置表中获取
        //if (BeanCtx.getSystemConfig("ArchivedGraphic").equals("1")) {
        if (false && BeanCtx.getSystemConfig("ArchivedGraphic").equals("1")) {
            // 只有在通用配置中配了才可以归档
    ...
    protected int archiveReportDoc(String docUnid) {
        ...
        if (rdb.hasRecord("select WF_OrUnid from BPM_InsUserList where DocUnid='" + docUnid + "'")) {
            ...
            int i = rdb.execSql(sql);
            if (i < 1) {
                BeanCtx.log("E", "MainDoc.archiveReportDoc", "归档用户实例数据时出错,记录没有插入到BPM_ReportUserList中(" + sql + ")");
            }
            return i;
        }
        else {
    ...
    public void deleteAllInsData(String docUnid) {
        ...
        rdb.execSql(sql);
        //sql = "delete from  BPM_SubFormData where DocUnid='" + docUnid + "'"; // 子表单数据，暂时不处理
        //rdb.execSql(sql);
    ...
    public void deleteArchivedData(String docUnid) {
        ...
        rdb.execSql(sql);
        //sql = "Delete From BPM_AttachmentLog Where DocUNID='" + docUnid + "'"; // 附件日记，暂不支持附件
        //rdb.execSql(sql);
        sql = "Delete From BPM_InsNodeList Where DocUNID='" + docUnid + "'"; // 节点实例
        rdb.execSql(sql);
        sql = "Delete From BPM_InsCopyUserList Where DocUNID='" + docUnid + "'"; // 待阅数据
        ...
        sql = "delete from BPM_DataSnapshot  Where DS_DocUnid='" + docUnid + "'"; // 数据快照
        rdb.execSql(sql);
        //sql = "delete from  BPM_SubFormData where DocUnid='" + docUnid + "'"; // 子表单数据，暂不支持
        //rdb.execSql(sql);
    }
